...
for i = 1:len_line
    ...
    for j = neib_joints
        cnt = cnt + 1;
        index_temp(cnt,:) = [j line_index(i,:)];        
    end
end

index1 = [[3:25]' repmat(26:27,23,1)];
index2 = [[1:4 6:8 10:25]' repmat([30 34],23,1)];
index = [index_temp;index1;index2];
end

function index = get_JLd_index03
joints = [4 6 7 8 10 11 12 14 15 18 19];
% joints = [joints joints+25];
nb_index = get_neighbour_index;

line_index_temp = [];
for j = joints
   nb_joints = [nb_index(j).neib nb_index(j).neib + 25]';
...
cnt = 1;
for i = 1:len_line
    nb_joints = [nb_index(line_index(i,1)).neib]';
    nb_joints = [nb_joints;nb_joints+25];
